...
def autosummary_toc_visit_html(self: nodes.NodeVisitor, node: autosummary_toc) -> None:
    """Hide autosummary pydantic toctree list in HTML output."""
            ...
            for _name, _sig, _summary, real_name in items:
                ...
                if docname not in self.env.found_docs:
                    if excluded(self.env.doc2path(docname, False)):
                        msg = __(
                            "autosummary pydantic references excluded document %r. Ignored."
                        )
                    else:
                        msg = __(
                            "autosummary pydantic: stub file not found %r. "
...
def process_generate_options(app: Sphinx) -> None:
    ...
    if suffix is None:
        logger.warning(
            __(
                "autosummary pydantic generates .rst files internally. "
                "But your source_suffix does not contain .rst. Skipped."
            )
        )
        return

    from pydantic_autosummary.generate import generate_autosummary_docs
    # from sphinxcontrib.autodoc_pydantic.generate import generate_autosummary_docs
