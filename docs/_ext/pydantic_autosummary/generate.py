...
from pydantic_autosummary import (
...
class AutosummaryRenderer:
    ...
    def __init__(self, app: Sphinx) -> None:
        ...
        system_templates_path = [
            os.path.join(
                os.path.abspath(os.path.dirname(__file__)), "templates", "autosummary"
            )
        ]
        ...
        logger.info(
            __("[autosummary pydantic] templates path: %s") % system_templates_path
        )
    ...
    def render(self, template_name: str, context: dict) -> str:
        ...
        try:
            ...
            try:
                ...
                template = self.env.get_template("%s.rst" % template_name)
            except TemplateNotFound:
                # fallback to base.rst
                template = self.env.get_template("base.rst")
...
class ModuleScanner:
    ...
    def is_skipped(self, name: str, value: Any, objtype: str) -> bool:
        try:
            ...
            logger.warning(
                __(
                    "autosummary pydantic: failed to determine %r to be documented, "
...
def generate_autosummary_docs(
    ...
    logger.info(
        __("[autosummary pydantic] generating autosummary for: %s")
        % ", ".join(showed_sources)
    )

    if output_dir:
        logger.info(__("[autosummary pydantic] writing to %s") % output_dir)

    if base_path is not None:
        sources = [os.path.join(base_path, filename) for filename in sources]
        sources = [
            source for source in sources if not "_ext" in source
        ]  # exclude all standard rst files from local _ext folder
    ...
    for entry in sorted(set(items), key=str):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    __(
                        "[autosummary pydantic] failed to import %s.\nPossible hints:\n%s"
                    ),
                    entry.name,
                    "\n".join(errors),
                )
